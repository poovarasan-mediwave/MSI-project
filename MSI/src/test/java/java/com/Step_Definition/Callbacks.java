...
public class Callbacks extends Base_Class {
	...
	public void user_launch_the_url_log_in_to_the_account() throws IOException, InterruptedException {
		...
		String url = FileReaderManager.getInstanceCR().getserviceuserurl();
		getDrive(url);
		...
		sleep(3000);
		ClickOnElement(si.getcallback().getRecaptchaframe());
		sleep(3000);
		// ClickOnElement(si.getcallback().getRecaptcha());
		sleep(3000);
		ClickOnElement(si.getcallback().getLogin());
		sleep(10000);
		
		si.getreference().GP_registration(url, url, url, url, url, url, url, url, url);
	...
	public void user_complete_the_eligibility_form() throws InterruptedException {
		ClickOnElement(si.getcallback().getStart());
		sleep(3000);
		ClickOnElement(si.gettopedithistory().getComplete());
		sleep(3000);
		ClickOnElement(si.getcallback().getYes());
		sleep(3000);
		ClickOnElement(si.getcallback().getPeriodyes());
		sleep(3000);
		ClickOnElement(si.getcallback().getPerioddatepicker());
		sleep(3000);
		ClickOnElement(si.getcallback().getCalendarback());
		sleep(3000);
		ClickOnElement(si.getcallback().getPerioddate());
		sleep(3000);
		ClickOnElement(si.getcallback().getYesornoperiodnext());
		sleep(3000);
		ClickOnElement(si.getcallback().getPerioddatepicker());
		sleep(3000);
		calendaryearback();
		sleep(3000);
		ClickOnElement(si.getcallback().getDateofbirth());
		sleep(3000);
		ClickOnElement(si.getcallback().getFeelsafeyes());
		sleep(3000);
		ClickOnElement(si.getcallback().getYesornoperiodnext());
		sleep(5000);
		ClickOnElement(si.getcallback().getRegisteredUKGPno());
		sleep(3000);
		ClickOnElement(si.getcallback().getRegisteredUKaddressno());
		sleep(3000);
		ClickOnElement(si.getcallback().getEnterregisteredUKGPno());
		sleep(3000);
		ClickOnElement(si.getcallback().getChoosecallbackslot());
		sleep(6000);
		ClickOnElement(si.getcallback().getAnytimecallback());
		sleep(8000);
		ClickOnElement(si.getcallback().getBookcallback());
		sleep(3000);
	}
