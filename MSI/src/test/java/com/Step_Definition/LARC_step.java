...
public class LARC_step extends Base_Class {
	...
	public void launch_the_service_user_url_for_larc_eligibility_flow(String string) throws Throwable {
		...
		wait(20);
		getDrive(FileReaderManager.getInstanceCR().getuserurl());
		sleep(2000);
	}
	...
	public void user_enter_the_serviceuser_email_and_click_the_next_button(String string) throws Throwable {
		enterText(si.gettop_page().getLogemail(), FileReaderManager.getInstanceCR().getLarcemail());
		wait(20);
	}
	...
	public void user_enter_the_serviceuser_password_and_click_recaptcha_and_next_button(String string)
			throws Throwable {
		enterText(si.gettop_page().getLogpass(), FileReaderManager.getInstanceCR().getLarcpass());

		sleep(2000);
		enterText(si.gettop_page().getPin(), FileReaderManager.getInstanceCR().getLarccode());
	}
	...
	public void user_click_logout_button_and_landed_in_login_page_for_larc_eligibility_flow() {
		ClickOnElement(si.gettop_page().getUserlogout());
	}
	...
	public void user_enter_the_memorable_word_and_click_login_button() throws Throwable {
		...
		sleep(2000);
		ClickOnElement(si.gettop_page().getRecaptcha());
		frame_out();
		sleep(4000);
		ClickOnElement(si.gettop_page().getLogin());
		sleep(14000);
	...
	public void user_click_the_start_button_in_the_dashboard_page() throws Throwable {
		// ClickOnElement(si.gettop_page().getStart());
		ClickOnElement(si.gettop_page().getCompletenow());
		sleep(2000);
	...
	public void user_click_yes_or_no_button_for_last_3months() throws Throwable {
		...
		// ClickOnElement(si.getlarc_page().getYesbtn());
//		ClickOnElement(si.gettop_page().getArrangecall());
//		ClickOnElement(si.gettop_page().getAnytime());
//		ClickOnElement(si.gettop_page().getCallback());
		sleep(5000);
		ClickOnElement(si.getlarc_page().getNobtn());
		sleep(1500);
	...
	public void user_click_select_the_treatment_type_and_click_next_button() throws Throwable {
		...
		ClickOnElement(si.getlarc_page().getChange());
		sleep(2000);

		ClickOnElement(si.getlarc_page().getSubmit());
		sleep(2000);
	...
	public void user_click_dob_and_enter_below_years_and_yes_or_no_for_sexually_active_button(Integer int1)
		...
		ClickOnElement(si.gettop_page().getDOB());

		String day = FileReaderManager.getInstanceCR().getDay();
		...
		ClickOnElement(si.gettop_page().getYesornoperiodnext());
	...
	public void user_click_yes_or_no_in_gp_details_and_registred_uk_address_and_uk_resident() throws Throwable {
		...
		ClickOnElement(si.gettop_page().getYesgp());

		// ClickOnElement(si.gettop_page().getNogp());
		sleep(2000);

		// ClickOnElement(si.getlarc_page().getNo2());

		// ClickOnElement(si.getlarc_page().getNo4());

		// ClickOnElement(si.getlarc_page().getNo6());
		ClickOnElement(si.getvas_page().getEntergp());
		sleep(2000);
		String findubrn = FileReaderManager.getInstanceCR().getLarcphone();
		enterText(si.getvas_page().getFindubrn(), findubrn);
		sleep(5000);
		ClickOnElement(si.getlarc_page().getAddress1());
	...
	public void user_click_yes_or_no_for_uk_address_and_moves_to_enter_gp_details_or_arrange_call_back()
			throws Throwable {
		// ClickOnElement(si.getlarc_page().getYes7());

		// ClickOnElement(si.getlarc_page().getNo8());
		sleep(6000);
